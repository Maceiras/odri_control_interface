...
#define assert_yaml_parsing(yaml_node, parent_node_name, child_node_name)                                                                                                               \
    if (!yaml_node[child_node_name]) {                                                                                                                                                  \
        ...
        oss << "Error: Wrong parsing of the YAML file from src file: [" << __FILE__ << "], in function: [" << __FUNCTION__ << "], line: [" << __LINE__ << ". Node [" << child_node_name \
...
#define assert_file_exists(filename)                                                                                                                                   \
    ...
    if (!f.good()) {                                                                                                                                                   \
        std::ostringstream oss;                                                                                                                                        \
        oss << "Error: Problem opening the file [" << filename << "], from src file: [" << __FILE__ << "], in function: [" << __FUNCTION__ << "], line: [" << __LINE__ \
            << ". The file may not exists.";                                                                                                                           \
        throw std::runtime_error(oss.str());                                                                                                                           \
    }                                                                                                                                                                  \
    assert(true)

std::shared_ptr<JointModules> JointModulesFromYaml(std::shared_ptr<MasterBoardInterface> robot_if, const YAML::Node& joint_modules_yaml) {
    ...
    assert_yaml_parsing(joint_modules_yaml, "joint_modules", "reverse_polarities");
    ...
    assert_yaml_parsing(joint_modules_yaml, "joint_modules", "lower_joint_limits");
    ...
    assert_yaml_parsing(joint_modules_yaml, "joint_modules", "upper_joint_limits");
    ...
    assert_yaml_parsing(joint_modules_yaml, "joint_modules", "max_joint_velocities");
    assert_yaml_parsing(joint_modules_yaml, "joint_modules", "safety_damping");
    return std::make_shared<JointModules>(robot_if, motor_numbers_vec, joint_modules_yaml["motor_constants"].as<double>(), joint_modules_yaml["gear_ratios"].as<double>(),
                                          joint_modules_yaml["max_currents"].as<double>(), rev_polarities_vec, lower_joint_limits_vec, upper_joint_limits_vec,
                                          joint_modules_yaml["max_joint_velocities"].as<double>(), joint_modules_yaml["safety_damping"].as<double>());
}

std::shared_ptr<IMU> IMUFromYaml(std::shared_ptr<MasterBoardInterface> robot_if, const YAML::Node& imu_yaml) {
    ...
    return std::make_shared<IMU>(robot_if, rotate_vector_vec, orientation_vector_vec);
}

std::shared_ptr<JointCalibrator> JointCalibratorFromYaml(std::shared_ptr<JointModules> joints, const YAML::Node& joint_calibrator) {
    ...
    for (std::size_t i = 0; i < joint_calibrator["search_methods"].size(); i++) {
        std::string method = joint_calibrator["search_methods"][i].as<std::string>();
        if (method == "AUTO") {
    ...
    assert_yaml_parsing(joint_calibrator, "joint_calibrator", "position_offsets");
    ...
    return std::make_shared<JointCalibrator>(joints, calib_methods, position_offsets_vec, joint_calibrator["Kp"].as<double>(), joint_calibrator["Kd"].as<double>(), joint_calibrator["T"].as<double>(),
...
std::shared_ptr<Robot> RobotFromYamlFile(const std::string& if_name, const std::string& file_path) {
    ...
    std::shared_ptr<MasterBoardInterface> robot_if = std::make_shared<MasterBoardInterface>(if_name);
    ...
    std::shared_ptr<JointCalibrator> calibrator = JointCalibratorFromYaml(joints, param["joint_calibrator"]);
...
std::shared_ptr<MasterBoardInterface> CreateMasterBoardInterface(const std::string& if_name, bool listener_mode) {
